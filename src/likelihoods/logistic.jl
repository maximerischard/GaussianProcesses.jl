...
function like(y::Vector{Float64},f::Vector{Float64})
    return -log(1+exp(-y.*f))
end

#1st derivative with respect to f
function grad1_likef(y::Vector{Float64},f::Vector{Float64})
        s   = min(0,f); 
...
function grad2_likef(y::Vector{Float64},f::Vector{Float64})
        s   = min(0,f); 
...
function grad3_likef(y::Vector{Float64},f::Vector{Float64})
        s   = min(0,f); 
        p   = exp(s)./(exp(s)+exp(s-f));                    
        d2lp_df2 = -exp(2*s-f)./(exp(s)+exp(s-f)).^2; # 2nd derivative of log likelihood
